        ...
        for i in data["items"]:
            if title:
                if title in i["snippet"]["title"]:
                    data_dict = {"thumbnail": i["snippet"]["thumbnails"]["maxres"]["url"],
...
def videolist():
    viddict = {worldcup: {'title': 'World Cup 2022 Qatar', 'link': 'worldcup/KokKw1esPL8',
                          'image': '/static/highlights/WC2022.jpg'},
               premierleague: {'title': 'Premier League 2022/23', 'link': 'premierleague/U0bndsJANvk',
        ...
        for term in search:
            for title in titles:
                if term in titles[title]['title']:
                    vidlist.append({"title": titles[title]['title'], "link": titles[title]['link'],
...
def worldcup(request, videolink):
    ...
    return render(request, "highlights/playepisode.html",
                  {"link": videolink, "data": infolist, "maintitle": maintitle, "maindescription": maindesc,
                   "publishdate": date, "page": "WC"})
